<?php
	...
	$file = fopen('imports/WB-Leads2.csv', 'r');
	...
	foreach ($clients as $client)
	{
		...
		if ($client['nom'] != '')
		{
			...
			if (! is_numeric($client['vide']))
			{
				$client['vide'] = '';
			//Insertion du client
			$req = "insert into wb_clients (id_type, id_statut, id_sstatut, id_departement, id_zone, date, nom, activite, adresse, cp, ville, arguments, capital, societecom, site1, site2, site3, site4, flyer, relance, maj) values (
'".mysqli_escape_string($sql_link, $id_type)."',
'".mysqli_escape_string($sql_link, $id_statut)."',
'".mysqli_escape_string($sql_link, $id_sstatut)."',
...
'".mysqli_escape_string($sql_link, 0)."'
			...
			{
			//Insertion du client
			$req = "insert into wb_clients (id, id_type, id_statut, id_sstatut, id_departement, id_zone, date, nom, activite, adresse, cp, ville, arguments, capital, societecom, site1, site2, site3, site4, flyer, relance, maj) values (
'".mysqli_escape_string($sql_link, (1000+$client['vide']))."',
'".mysqli_escape_string($sql_link, $id_type)."',
'".mysqli_escape_string($sql_link, $id_statut)."',
'".mysqli_escape_string($sql_link, $id_sstatut)."',
'".mysqli_escape_string($sql_link, $id_departement)."',
